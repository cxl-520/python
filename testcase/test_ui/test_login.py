...
# 新增：给测试类添加整体描述
@allure.description("该测试类覆盖Saucedemo平台登录的全场景校验：正确账号退出、错误账号/密码、空账号/空密码")
class TestLogin:
    # 后面用公共夹具了
    ...
    # 新增：用例级详细描述
    @allure.description("验证正确账号密码登录后，点击退出登录能返回登录页，通过URL断言校验退出结果")
    # 核心：参数化装饰器
    ...
    def test_login_001(self, username, password, expected_msg, test_login1):
        with allure.step("步骤1：输入账号密码并登录"):  # 新增：步骤拆分
            # 登录正确账号
            driver = test_login1
            ...
            # 新增：登录后截图附件
            allure.attach(driver.get_screenshot_as_png(), name="登录成功后页面",
                          attachment_type=allure.attachment_type.PNG)

        with allure.step("步骤2：点击侧边栏并退出登录"):  # 新增：步骤拆分
            driver.find_element(By.XPATH, '//*[@id="react-burger-menu-btn"]').click()
            sleep(5)
            driver.find_element(By.XPATH, '//*[@id="logout_sidebar_link"]').click()
            # 新增：退出后截图附件
            allure.attach(driver.get_screenshot_as_png(), name="退出登录后页面",
                          attachment_type=allure.attachment_type.PNG)

        with allure.step("步骤3：断言退出登录后的URL"):  # 新增：步骤拆分
            # 获取当前页面的url，做断言，判断是不是已经退出登录
            current_url = driver.current_url
            assert current_url == expected_msg
            # 新增：断言结果记录
            allure.attach(f"实际URL：{current_url} | 预期URL：{expected_msg}", name="断言结果",
    ...
    # 新增：用例描述
    @allure.description("验证错误账号+正确密码登录时，能弹出预期的错误提示文案")
    def test_login_002(self, username, password, expected_msg, test_login1):
        with allure.step("步骤1：输入错误账号和正确密码"):  # 新增：步骤拆分
            # 登录账号错误
            driver = test_login1
            login_name = driver.find_element(By.XPATH, '//*[@id="user-name"]')
            login_name.send_keys(username)
            ...
            driver.find_element(By.XPATH, '//*[@id="login-button"]').click()
            # 新增：错误提示页面截图
            allure.attach(driver.get_screenshot_as_png(), name="错误账号登录提示页",
                          attachment_type=allure.attachment_type.PNG)

        with allure.step("步骤2：获取错误提示并断言"):  # 新增：步骤拆分
            text = driver.find_element(By.XPATH, '//*[@id="login_button_container"]/div/form/div[3]/h3').text
            print(text)
            assert text == expected_msg
            # 新增：断言结果记录
            allure.attach(f"实际提示：{text} | 预期提示：{expected_msg}", name="断言结果",
                          attachment_type=allure.attachment_type.TEXT)
    ...
    # 新增：用例描述
    @allure.description("验证正确账号+错误密码登录时，能弹出预期的错误提示文案")
    def test_login_003(self, username, password, expected_msg, test_login1):
        with allure.step("步骤1：输入正确账号和错误密码"):  # 新增：步骤拆分
            # 登录密码错误
            driver = test_login1
            login_name = driver.find_element(By.XPATH, '//*[@id="user-name"]')
            login_name.clear()
            ...
            driver.find_element(By.XPATH, '//*[@id="login-button"]').click()
            # 新增：错误提示页面截图
            allure.attach(driver.get_screenshot_as_png(), name="错误密码登录提示页",
                          attachment_type=allure.attachment_type.PNG)
    ...
    def test_login_004(self, username, password, expected_msg, test_login1):
        with allure.step("步骤1：输入空账号和非空密码"):  # 新增：步骤拆分
            ...
            driver.find_element(By.XPATH, '//*[@id="login-button"]').click()
            # 新增：错误提示页面截图
            allure.attach(driver.get_screenshot_as_png(), name="空账号登录提示页",
                          attachment_type=allure.attachment_type.PNG)

        with allure.step("步骤2：获取必填提示并断言"):  # 新增：步骤拆分
            text = driver.find_element(By.XPATH, '//*[@id="login_button_container"]/div/form/div[3]/h3').text
            print(text)
            assert text == expected_msg
            # 新增：断言结果记录
    ...
    @allure.severity(allure.severity_level.NORMAL)
    # 新增：用例描述
    @allure.description("验证非空账号+空密码登录时，能弹出密码必填的预期提示文案")
    @pytest.mark.parametrize("username,password,expected_msg", [
        ("standard_user", "", "Epic sadface: Password is required")
    ])
    def test_login_005(self, username, password, expected_msg, test_login1):
        with allure.step("步骤1：输入非空账号和空密码"):  # 新增：步骤拆分
            # 密码为空
            driver = test_login1
            login_name = driver.find_element(By.XPATH, '//*[@id="user-name"]')
            login_name.clear()
            ...
            driver.find_element(By.XPATH, '//*[@id="login-button"]').click()
            # 新增：错误提示页面截图
            allure.attach(driver.get_screenshot_as_png(), name="空密码登录提示页",
                          attachment_type=allure.attachment_type.PNG)
...
if __name__ == "__main__":
    # 仅修正pytest.main传参格式（否则无法生成Allure报告，无其他修改）
    pytest.main([
        '-s',
        "test_login.py",
        '-v',  # 新增：详细输出（不影响原有逻辑）
        '--alluredir=testcase/allure-results',  # 新增：指定Allure报告输出目录（和你注释的命令一致）
        '--clean-alluredir'  # 新增：清空旧报告（避免缓存）
    ])
